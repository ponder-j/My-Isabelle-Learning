theory HSV_tasks_2019_solutions2 imports Complex_Main begin

section ‹Task 1: proving that 2 * sqrt 2 is irrational.›
...
proof auto
  assume *: "2 * sqrt 2 ∈ ℚ"
  have "2 ∈ ℚ" by simp
  with * have "2 * sqrt 2 / 2 ∈ ℚ" using Rats_divide by blast
  hence "sqrt 2 ∈ ℚ" by simp
  with sqrt2_irrational show False by simp
qed

section ‹Task 2: L-numbers.›
...
section ‹Task 4: the opt_NOT optimisation leaves no consecutive NOTs in the circuit.›
...
text ‹We can prove that the optimiser does its job: that the optimised 
circuit has no consecutive NOT gates.›

fun has_double_NOT where
  "has_double_NOT (NOT (NOT c)) = True"
| "has_double_NOT (NOT c) = has_double_NOT c"
| "has_double_NOT (AND c1 c2) = (has_double_NOT c1 ∨ has_double_NOT c2)"
| "has_double_NOT (OR c1 c2) = (has_double_NOT c1 ∨ has_double_NOT c2)"
| "has_double_NOT _ = False"

theorem opt_NOT_is_complete: "¬ has_double_NOT (opt_NOT c)"
  by (induct c rule: opt_NOT.induct, auto)

section ‹Task 5: Removing NOT-NOT is idempotent.›

text ‹We can prove that the optimiser is 'idempotent': applying it twice is the 
same as applying it just once.›

theorem opt_NOT_is_idempotent: "opt_NOT (opt_NOT c) = opt_NOT c"
  by (induct c rule: opt_NOT.induct, auto)

section ‹Task 6: Applying De Morgan's laws is sound.›

text ‹Optimising based on De Morgan's laws›
...
text ‹The following non-theorem is easily contradicted, e.g. 
  using c = AND (NOT (NOT TRUE)) (NOT (NOT TRUE)). Therefore, 
  opt_DM isn't idempotent.›

theorem opt_DM_is_idempotent: "opt_DM (opt_DM c) = opt_DM c"
  oops

section ‹Task 7: Applying both optimisations successively is sound.›
...
theorem both_sound: "simulate (opt_DM (opt_NOT c)) ρ = simulate c ρ"
  apply (subst opt_DM_is_sound)
  apply (subst opt_NOT_is_sound)
  apply auto
  done

section ‹Task 8: Neither optimisation increases the circuit's area.›

text ‹The following function calculates the area of a circuit (i.e. number of gates).›
...
theorem opt_NOT_doesnt_increase_area: "area (opt_NOT c) ≤ area c"
  by (induct rule: opt_NOT.induct, auto)

theorem opt_DM_doesnt_increase_area: "area (opt_DM c) ≤ area c"
  by (induct rule: opt_DM.induct, auto)

section ‹Task 9: Neither optimisation increases the circuit's delay.›

text ‹Delay (assuming all gates have a delay of 1, and constants and inputs are instantaneous)›
...
text ‹We can prove that our optimisations never increase the maximum delay of a circuit.›

theorem opt_NOT_doesnt_increase_delay: "delay (opt_NOT c) ≤ delay c"
  by (induct rule: opt_NOT.induct, auto)

theorem opt_DM_doesnt_increase_delay: "delay (opt_DM c) ≤ delay c"
  by (induct rule: opt_DM.induct, auto)

section ‹Task 10: Constant folding.›

text ‹Here is an optimiser that performs 'constant folding': wherever it sees a gate with
...
First, we define how it translates NOT gates, AND gates, and OR gates:›
...
text ‹The actual optimiser just runs the above rewrites on each NOT, AND, and OR gate.›
...
text ‹To prove the constant folder sound, first consider the soundness of the sub-cases.›

lemma opt_CF_NOT_is_sound: "simulate (opt_CF_NOT c) ρ = simulate (NOT c) ρ"
  by (cases c, auto)

lemma opt_CF_AND_is_sound: "simulate (opt_CF_AND c1 c2) ρ = simulate (AND c1 c2) ρ"
  by (induct c1 c2 rule: opt_CF_AND.induct, auto)

lemma opt_CF_OR_is_sound: "simulate (opt_CF_OR c1 c2) ρ = simulate (OR c1 c2) ρ"
  by (induct c1 c2 rule: opt_CF_OR.induct, auto)

theorem opt_CF_is_sound: "simulate (opt_CF c) ρ = simulate c ρ"
  by (induct c rule: opt_CF.induct,
      auto simp add: opt_CF_NOT_is_sound opt_CF_AND_is_sound opt_CF_OR_is_sound)

text ‹To get an idea of whether the constant folder is complete, we can see whether it reduces a
circuit with only constant inputs into a single TRUE or FALSE constant.  First, define what it
means for a circuit to have no inputs, and to be only a constant.›

fun no_inputs where
  "no_inputs (NOT c) = no_inputs c"
| "no_inputs (AND c1 c2) = (no_inputs c1 ∧ no_inputs c2)"
| "no_inputs (OR c1 c2) = (no_inputs c1 ∧ no_inputs c2)"
...
lemma opt_CF_NOT_doesnt_increase_area: "area (opt_CF_NOT c) ≤ area (NOT c)"
  by (cases c, auto)

lemma opt_CF_AND_doesnt_increase_area: "area (opt_CF_AND c1 c2) ≤ area (AND c1 c2)"
  by (induct c1 c2 rule: opt_CF_AND.induct, auto)

lemma opt_CF_OR_doesnt_increase_area: "area (opt_CF_OR c1 c2) ≤ area (OR c1 c2)"
  by (induct c1 c2 rule: opt_CF_OR.induct, auto)

theorem opt_CF_doesnt_increase_area: "area (opt_CF c) ≤ area c"
proof (induct rule: opt_CF.induct)
  case (1 c) 
  have "area (opt_CF (NOT c)) 
                 = area (opt_CF_NOT (opt_CF c))" by simp
  also have "... ≤ area (NOT (opt_CF c))" 
    using opt_CF_NOT_doesnt_increase_area by assumption
  also have "... ≤ area (NOT c)" using 1 by simp
  finally show ?case by assumption
next
  case (2 c1 c2)
  have "area (opt_CF (AND c1 c2)) 
                 = area (opt_CF_AND (opt_CF c1) (opt_CF c2))" by simp
  also have "... ≤ area (AND (opt_CF c1) (opt_CF c2))" 
    using opt_CF_AND_doesnt_increase_area by assumption
  also have "... ≤ area (AND c1 c2)" using 2 by simp
  finally show ?case by assumption
next
  case (3 c1 c2)
  have "area (opt_CF (OR c1 c2)) 
                 = area (opt_CF_OR (opt_CF c1) (opt_CF c2))" by simp
  also have "... ≤ area (OR (opt_CF c1) (opt_CF c2))" 
    using opt_CF_OR_doesnt_increase_area by assumption
  also have "... ≤ area (OR c1 c2)" using 3 by simp
  finally show ?case by assumption
qed simp+

lemma opt_CF_NOT_doesnt_increase_delay: "delay (opt_CF_NOT c) ≤ delay (NOT c)"
  by (cases c, auto)

lemma opt_CF_AND_doesnt_increase_delay: "delay (opt_CF_AND c1 c2) ≤ delay (AND c1 c2)"
  by (induct c1 c2 rule: opt_CF_AND.induct, auto)

lemma opt_CF_OR_doesnt_increase_delay: "delay (opt_CF_OR c1 c2) ≤ delay (OR c1 c2)"
  by (induct c1 c2 rule: opt_CF_OR.induct, auto)

theorem opt_CF_doesnt_increase_delay: "delay (opt_CF c) ≤ delay c"
proof (induct rule: opt_CF.induct)
  case (1 c) 
  have "delay (opt_CF (NOT c)) 
                 = delay (opt_CF_NOT (opt_CF c))" by simp
  also have "... ≤ delay (NOT (opt_CF c))" 
    using opt_CF_NOT_doesnt_increase_delay by assumption
  also have "... ≤ delay (NOT c)" using 1 by simp
  finally show ?case by assumption
next
  case (2 c1 c2)
  ...
  also have "... ≤ delay (AND c1 c2)" using 2 by auto
  finally show ?case by assumption
next
  case (3 c1 c2)
  ...
  also have "... ≤ delay (OR c1 c2)" using 3 by auto
  finally show ?case by assumption
qed simp+

section ‹Task 11: Adding fan-out.›

text ‹Please see separate file.›
