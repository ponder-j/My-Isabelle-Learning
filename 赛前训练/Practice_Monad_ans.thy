...
(* 定义 bind: 串联两个计算 *)
definition my_bind :: "('a, 's) state \<Rightarrow> ('a \<Rightarrow> ('b, 's) state) \<Rightarrow> ('b, 's) state" (infixl ">>=" 60) where
  "my_bind m f = (\<lambda>s. let (a, s') = m s in f a s')"
...
lemma monad_left_identity:
  ...
  unfolding return_def my_bind_def
