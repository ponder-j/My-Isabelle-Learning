...
primrec nth :: "'a array \<Rightarrow> nat \<Rightarrow> 'a option"("_[_]")
  where "nth Null i = None" |
        "nth (Conn a arr) i = (case i of 0 \<Rightarrow> Some a | Suc k \<Rightarrow> nth arr k)"

primrec array_update :: "'a array \<Rightarrow> nat \<Rightarrow> 'a \<Rightarrow> 'a array" ("_[_] := _")
  where "array_update Null i v = Null" |
        "array_update (Conn a arr) i v = 
